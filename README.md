...
- Admin: kelola produk (CRUD + stok + foto), tambah pengguna baru & atur role, laporan transaksi (filter per kasir + detail, cetak ulang struk, void/batalkan transaksi dengan stok otomatis dikembalikan) & total penjualan harian.
- Row Level Security di Supabase: kasir hanya melihat transaksinya sendiri, admin melihat semua data.

## 1. Setup Supabase

1. Buat project baru di [supabase.com](https://supabase.com).
2. Buka **SQL Editor**, jalankan isi file `supabase/schema.sql` untuk membuat tabel, role, RLS policy, trigger pembuatan profile otomatis, dan storage bucket `products` untuk foto produk. Jika project Supabase sudah ada sebelumnya (sudah pernah jalankan versi lama `schema.sql`), jalankan juga `supabase/migration_diskon_foto.sql` (kolom diskon & bucket foto produk) dan `supabase/migration_void_refund.sql` (izin admin membatalkan transaksi).
