...
export default async function RiwayatPage() {
  ...
  const totalToday = (transactions ?? [])
    .filter((tx) => tx.status === "paid" && new Date(tx.created_at).toDateString() === new Date().toDateString())
  ...
  return (
    <div className="space-y-4 p-4">
      <div className="rounded-lg border border-orange-500/20 bg-neutral-900 p-4 shadow-sm">
        <p className="text-sm text-neutral-400">Total penjualan saya hari ini</p>
